...
def get_market_data():
    try:
        ...
        r = requests.get(url, params={"interval": "1h", "range": "2d"},
...
def ai_decision(market, news, fear_greed, balance):
    try:
        prompt = (
            ...
            f"BTC: ${market.get('price')} | RSI: {market.get('rsi')} | "
            f"MACD: {market.get('macd')} | 24h: {market.get('change24h')}%\n"
            f"Fear&Greed: {fear_greed}\nNews:\n{news}\n"
            f"Bilancio: ${balance}\n\n"
            "Scegli UNA opzione e rispondi SOLO con questo JSON:\n"
            '{"action":"BUY","reason":"motivo"}\n'
            'oppure {"action":"SELL","reason":"motivo"}\n'
            'oppure {"action":"HOLD","reason":"motivo"}'
        )
        r = requests.post(
