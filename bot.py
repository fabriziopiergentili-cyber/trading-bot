...
last_update_id    = 0
trailing_stops    = {}
trade_history     = []        # tutti i trade chiusi
last_daily_report = None      # data ultimo report giornaliero
DAILY_REPORT_HOUR = 8         # ora del report
...
def record_trade(side, entry, exit_price, pnl, reason=""):
    """Registra un trade chiuso in trade_history."""
...
def get_stats():
    """Calcola statistiche sui trade chiusi."""
...
def format_stats_message(balance):
    """Formatta messaggio stats per Telegram."""
    stats     = get_stats()
    ...
    if not stats:
        msg += "⏳ Nessun trade chiuso ancora — il bot sta raccogliendo dati."
        return msg
    ...
    try:
        ...
        for update in updates:
            ...
            if chat_id and text.startswith("/start"):
                ...
                balance = get_balance()
                msg_out = format_stats_message(balance)
                send_message(msg_out, chat_id=chat_id)
...
def place_order(side, price, balance):
    try:
        ...
        is_buy   = side == "BUY"
        limit_price = round(price * 0.9998, 1) if is_buy else round(price * 1.0002, 1)
...
def update_trailing_stop(pos, price):
    ...
    if is_long and price >= entry * 1.02:
        ...
        if new_sl > current_sl:
            log(f"Trailing stop LONG: ${current_sl:,.2f} → ${new_sl:,.2f}")
            cancel_open_orders()
            try:
                sl_result = exchange_close.order(
                ...
                if sl_result and sl_result.get("status") == "ok":
                    ...
                    log(f"✅ Trailing SL aggiornato @ ${new_sl:,.2f}")
        ...
        new_sl = round(price * (1 + TRAIL_PCT), 1)
        if new_sl < current_sl:
            log(f"Trailing stop SHORT: ${current_sl:,.2f} → ${new_sl:,.2f}")
            cancel_open_orders()
            try:
                sl_result = exchange_close.order(
                ...
                if sl_result and sl_result.get("status") == "ok":
                    trailing_stops[SYMBOL]["sl"] = new_sl
                    log(f"✅ Trailing SL SHORT aggiornato @ ${new_sl:,.2f}")
...
def monitor_positions():
    ...
    for pos in positions:
        ...
        log(f"Pos {pos['side']} | Entry: ${entry:,.2f} | PnL: ${pnl:.4f} | Price: ${price:,.2f}")
...
7. You can also respond with CLOSE to close an existing position early.
...
def main():
    ...
    log("FIX attivi: bilancio spot, limit orders, SL/TP reali, trailing stop, EMA vera, stats")
    ...
    while True:
        try:
            ...
            if (now_dt.hour == DAILY_REPORT_HOUR and
                ...
                balance = get_balance()
                report  = f"🌅 *Report Giornaliero — {now_dt.strftime('%d/%m/%Y')}*\n\n"
                report += format_stats_message(balance)
                notify(report)
            ...
            if now - last_analysis >= INTERVAL:
                ...
                if not market:
                    ...
                    if action == "BUY" and balance >= 10 and not positions:
                        ...
                        pos = positions[0]
                        decision_history[-1]["outcome"] = f"Position already open, PnL=${pos['pnl']:.4f}"
                        last_analysis = now
